...
const SourceForm = ({ source, onSave, onCancel, isNew }) => {
...
// --- Sources Page ---
const SourcesPage = ({ onEdit, onAdd }) => (
  <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <Btn outline onClick={()=>alert("→ POST /api/actions/check-all")}>▶ Проверить все</Btn>
      </div>
    </div>
        ...
        <tbody>
          {DEMO_SOURCES.map((s,ri)=>(
            <tr key={ri} style={{borderBottom:`1px solid ${T.border}`,transition:"background .15s",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background=T.bgHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"} onClick={()=>onEdit(s)}>
              ...
              <td style={{padding:"10px"}}><Btn small outline onClick={e=>{e.stopPropagation();onEdit(s)}}>✎</Btn></td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function App() {
  ...
  const handleSaveSource = (formData) => { alert(`→ ${isNewSource?"POST":"PUT"} /api/sources/${formData.code}\n\n${JSON.stringify(formData,null,2).substring(0,500)}…`); setPage("sources"); };
  ...
  return <div style={{display:"flex",height:"100vh",background:T.bg,fontFamily:T.sans,color:T.text}}>
    ...
    <div style={{flex:1,overflow:"auto",padding:24}}>
      ...
      {page==="sources"&&<SourcesPage onEdit={handleEditSource} onAdd={handleNewSource} />}
      {page==="source-form"&&<SourceForm source={editSource} isNew={isNewSource} onSave={handleSaveSource} onCancel={handleCancelForm} />}
